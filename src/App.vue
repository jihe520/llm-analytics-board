...
<template>
  <div class="flex min-h-screen bg-gray-100 relative">
    <!-- 左侧边栏切换按钮 -->
    <button @click="toggleLeftSidebar" class="fixed left-4 top-4 z-50 md:hidden bg-white p-2 rounded-lg shadow-lg">
      <Icon icon="mdi:menu" class="w-6 h-6" />
    </button>

    <!-- 右侧边栏切换按钮 -->
    <button @click="toggleRightSidebar" class="fixed right-4 top-4 z-50 md:hidden bg-white p-2 rounded-lg shadow-lg">
      <Icon icon="mdi:cog" class="w-6 h-6" />
    </button>

    <!-- 左侧边栏 -->
    <Sidebar position="left" :class="[
